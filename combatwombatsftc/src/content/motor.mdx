...
description: "A comprehensive guide to understanding and using motors in FTC."
date: "2024-04-22"
...
Additionally, you can set the direction the motor will turn when given an input. When given as **input of 1, with the shaft facing away the user** when the controller is held normally, this will move towards the player:

```java
motor.setDirection(DcMotor.Direction.REVERSE);
```

While this will move away:

```java
motor.setDirection(DcMotor.Direction.FORWARD);
```

Additionally, you can choose what will happen when there is no input given. Either you can make the robot slow to a halt, or do nothing if the robot is still:
